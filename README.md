# @devtea2027/aliquam-aut-aut-aliquid

Get environment variables exposed by CI services.

[![Build Status](https://github.com/devtea2027/aliquam-aut-aut-aliquid/workflows/Test/badge.svg)](https://github.com/devtea2027/aliquam-aut-aut-aliquid/actions?query=workflow%3ATest+branch%3Amaster)
[![npm latest version](https://img.shields.io/npm/v/@devtea2027/aliquam-aut-aut-aliquid/latest.svg)](https://www.npmjs.com/package/@devtea2027/aliquam-aut-aut-aliquid)
...
```bash
$ npm install --save @devtea2027/aliquam-aut-aut-aliquid
```

## Usage

```js
import envCi from "@devtea2027/aliquam-aut-aut-aliquid";
...
To work around this limitation, `@devtea2027/aliquam-aut-aut-aliquid` look for the remote branches having the same `HEAD` as the local
detached `HEAD` to determine the branch from which the detached `HEAD` was created.
In the rare case where there is multiple remote branches with the same `HEAD` as the local detached `HEAD`, `@devtea2027/aliquam-aut-aut-aliquid`
...
a `git` command.
Therefore, in the case of Pull Request builds, `@devtea2027/aliquam-aut-aut-aliquid` will not be able to determine the `branch` and `prBranch`
properties.
...
the target branch.
Therefore, in the case of Pull Request builds, `@devtea2027/aliquam-aut-aut-aliquid` will not be able to determine the `branch` property.
...
originating branch and not provide a target. Therefore `@devtea2027/aliquam-aut-aut-aliquid` will not be able to determine the `prBranch` property
...
and [gitlab-plugin](https://github.com/jenkinsci/gitlab-plugin). Therefore `@devtea2027/aliquam-aut-aut-aliquid` will set `isPr`, `pr` and `prBranch`
